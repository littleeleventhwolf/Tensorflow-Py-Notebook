...
# 4-D tensor to use it in a convolutional graph. If one component of `shape`
# is the special value -1, the size of that dimension is computed so that
# the total size remains constant. Since we haven't defined the batch 
# dimension's shape yet, we use -1 to denote this dimension should not
# change size.
...
# we define our stride as [batch, height, width, channels]
# instead of pooling, we use strides of 2 and more layers
# with smaller filters.
h_conv1 = tf.nn.relu(tf.nn.conv2d(input=h_trans, filter=W_conv1, 
	                              strides=[1, 2, 2, 1], padding='SAME')
                     + b_conv1)

# And just like the first layer, add additional layers to creat a deep net.
n_filters_2 = 16
...
# We now create a new session to actually perform the initialization of the variables:
sess = tf.Session()
