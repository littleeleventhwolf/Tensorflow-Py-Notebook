...
class Neural_Styler(object):
	...
	"""
	def __init__(self, base_img_path, style_img_path, output_img_path, output_width, convnet,
				 content_weight, style_weight, tv_weight, content_layer, style_layers, iterations):
		"""
		...
		[3] L_BFGS requiers that loss and grad be two functions so we create
		...
		content_loss = self.content_weight * \
		        feature_reconstruction_loss(base_image_features, combination_features)
		...
		# compute gradients of output_img with respect to loss
	...
	def style(self):
		...
		for i in range(self.iterations):
			...
			toc = time.time()
			print('\t\tImage saves as ', fname)
			print('\t\tLoss: {:.2e}, Time: {} seconds'.format(float(min_val), float(toc-tic)))
